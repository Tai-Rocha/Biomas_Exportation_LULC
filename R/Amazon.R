######################################################################
## Code to build graphic Amazon exportation data from 1997 - 2021
## Author : Tainá Rocha
## R verison 4.2.2
...
## Biome shape
amazonshape = geobr::read_biomes(year = 2019, simplified = TRUE, showProgress = TRUE) |>
  filter(name_biome == "Amazônia")

## Join munishape and amazonhape
AMZmunishape = st_join(munishape, amazonshape, join = st_intersects) |> 
  st_make_valid()
...
exportmunishapeJOIN |> 
  ...
  geom_sf(data = amazonshape, color = "gray", fill = NA, size = 0.1)  +
  # Add the kgLiquid per municipalities
  geom_sf(aes(fill = kgLiquido)) +
  # Add the legend for the kgLiquid scale
...
## Join AMZmunishape and haAnthropic

MuniHAanthropic = left_join(AMZmunishape, haAnthropic, by = "name_muni", multiple = "all")  |> 
  na.omit() |> 
  st_make_valid()
...
## 2019 - 2021 - not work f0r 2020 and 2021
...
MuniHAnatural = left_join(AMZmunishape, haNatural, by = "name_muni", multiple = "all")  |> 
  na.omit() |> 
  st_make_valid()
...
# Line plots
ggplot(haNonF, aes(x = year, y = Ha, color = level_2)) +
  geom_line() +
