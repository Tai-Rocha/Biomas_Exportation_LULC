...
nonF + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                          panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))


## Join mapbiomas 

NonForestMun = mapbiomasraw |>
  separate(city, into = c("city", "state", "biome"), sep = " - ") |>
  rename(name_muni = city) |> 
  filter(level_1 == "2. Non Forest Natural Formation")  |> 
  select(-color) |>
...
bindedNonF = left_join(NonForestMun, Muni, by = "name_muni") |> 
  st_as_sf() |> 
  st_make_valid()

# 1997_2000

png(file="nonF_1997_2000.png",
